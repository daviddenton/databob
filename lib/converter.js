...
var arrayConverter = function (arrayValue) {
    return function () {
        return _.map(arrayValue, function(value) {
            var valueConverter = converter(value);
            console.log(df.generate(valueConverter));
            return _.isFunction(valueConverter) && !_.isArray(value) ? df.generate(valueConverter): valueConverter;
        });
    };
};
