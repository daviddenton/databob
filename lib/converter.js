...
var arrayConverter = function (arrayValue) {
    return function () {
        return _.map(arrayValue, function (elementValue) {
            var elementConverter = converter(elementValue);
            return _.isFunction(elementConverter) ? elementConverter() : elementConverter;
        });
    };
};
...
var objectConverter = new fcb.FallbackConverterBuilder().thenMatch(_.isDate, function () {
        return function () {
            return new Date();
        };
...
var numberConverter = new fcb.FallbackConverterBuilder().thenMatch(_.partial(_.isEqual, Infinity), _.constant(Infinity)).thenMatch(_.partial(_.isEqual, -Infinity), _.constant(-Infinity)).thenMatch(_.isNaN, _.constant(NaN)).buildWithDefault(function (numberValue) {
...
var converter = new fcb.FallbackConverterBuilder().thenMatch(_.isNull, _.constant(undefined)).thenMatch(_.isUndefined, _.constant(undefined)).thenMatch(_.isBoolean, booleanConverter).thenMatch(_.isString, stringConverter).thenMatch(_.isArray, arrayConverter).thenMatch(_.isNumber, numberConverter).thenMatch(_.isObject, objectConverter).buildWithDefault(function (value) {
